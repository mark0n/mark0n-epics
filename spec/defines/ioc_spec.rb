    ...
    service_providers.each do |serv_prov|
      context "on #{os} with service provider #{serv_prov}" do
        ...
        if serv_prov == 'systemd'
          ...
          it { is_expected.to compile.and_raise_error("epics: Service provider #{serv_prov} is not supported, yet. Pull-requests welcome ;-)") }
        end
